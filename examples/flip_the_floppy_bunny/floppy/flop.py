...
'''# flop.py
    ...
    ## Floppy's Quest for Swedish Flops (Demo)
    
    Based on the py game 'Pydza' on Sourceforge [[ https://sourceforge.net/projects/pydza/ ]].
    It can use the PydzEd level editor, also with Pydza on Sourceforge, and both
    are in the public domain, along with their content.
    
    The sprites (except the chef) are CC0 by [[ kenney.nl ]]. I made the chef,
    and I don't know WHERE I could've gotten the idea for the design! (It's GPL3.)
    
    My tweak of the source code (mostly "translation" of some comments) is released
    under the MIT license. However, use of the chef sprite makes this game is GPL3
    unless you delete the chef graphics.
    
    ## Story:
    
    Help a couple bunnies -- Flo and her brother Flip -- get Swedish Flops for
    their parents, Philip and Phillis!
    
    It is currently one player only (Flip, ironically). The Ms. Flop two-player
    version is coming soon... A useful quick-build "Pydza 2D Engine" module is
    also coming soon, for use with pygame or pygame-zero or the ThorPy GUI library...
'''
...
class Actor(pygame.sprite.Sprite):
    ...
    def touch(self, other):
        ...
        It says, "Function something for an actor (object) to
...
class WeakBrick(Brick):
    """Hey, je tombe.
        ...
        Hmm, I don't know how, but it says that it knows I suck.
    """
    def __init__(self, posX, posY, image):
...
class Pike(Brick):
    """Malheuresement, le joueur n'est pas un fakir.
        ...
        This is just swears. DON'T READ IT!
    """
    def top_touch(self, other):
        '''Spikes kill the player. But only when touching the top!
        '''
...
# Lethal Brick:
class MadBrick(Pike):
    '''MadBricks kill the player when it touches ANY side!'''
    def bottom_touch(self, other):
        self.top_touch(other)
...
class Mover(Brick):
    """Ca s'en va et ça revient...
        ...
        This is a quote by Sartre. Not important.
    """
...
class Pizza(Actor):
    ...
    #images = load_images('piz1.gif','piz2.gif','piz3.gif','piz4.gif','piz5.gif','piz6.gif','piz7.gif','piz8.gif','piz9.gif')
    image = load_image('flop.gif')
    #for x in range(0,7):    images.append(images[7-x]) # Duplicate to represent them facing opposite direction.
    #image = images[0]
    #animcycles = 5
    animcycles = 1
    frame = 0
...
class Pizzaman(Actor):
    ...
    image = load_image('flopstand.gif')
...
class Player(Actor):
    ...
    def __init__(self):
        ...
        self.images = load_images('bunny1_walk1.gif','bunny1_walk2.gif')
        for x in range(0, 2):
            ...
            self.images.append(pygame.transform.flip(self.images[x], 1, 0))
        img = load_image('bunny1_jump.gif')
        self.jumpimage = [img, pygame.transform.flip(img, 1, 0)]
        img = load_image('bunny1_ready.gif')
        self.fallimage = [img, pygame.transform.flip(img, 1, 0)]
        img = load_image('bunny1_stand.gif')
        self.standimage = [img, pygame.transform.flip(img, 1, 0)]
        self.deadimage = load_image('bunny1_hurt.gif')
        self.image = self.standimage[0]
        self.frame = 0
        #self.animcycles = 3
        self.animcycles = 2 # Fix all these types of things to 2 frames of animation.
        # Sons
    ...
    def move(pawn, others, dir, run=0):
        ...
        if not(pawn.jumping or pawn.falling):
            if dir != 0:
                ...
                frame = int(pawn.frame/pawn.animcycles%2) # Only 2 frames of anim now...
                if dir == -1:
                    #frame += 3
                    frame += 2

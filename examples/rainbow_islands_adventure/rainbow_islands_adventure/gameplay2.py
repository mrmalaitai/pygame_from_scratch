...
class Player():
    def __init__(self,pos):
        ...
        for sprites in self.sprites:
            if sprites != 'sprites_lenght':
               if type(self.sprites[sprites]) is list:
                  for i in range(len(self.sprites[sprites])):
                      self.sprites[sprites][i].convert()
               else:
        ...
        self.on_ground=True
        self.fall_through=0
        self.bounding_box=Rect([15,11,35,47])
    ...
    def idle(self):
        ...
        if up:
           if down:
              if self.on_ground and self.pos[1]+11+47<floor_pos:
                 self.fall_through=3
                 self.extra_yvel=5
...
def main():
    ...
    while True:
        ...
        #test if we collide with level ends
        if player_x_axis_collision_rect[0]<32:
           player_x_axis_collision_rect[0]=32
           if player.current_state=="rainbow_walk":
              player_y_axis_collision_rect[1]-=player.yvel
              player.yvel=0
              player.extra_yvel=0           
        elif player_x_axis_collision_rect[0]+player.bounding_box[2]>total_level_width-32:
           player_x_axis_collision_rect[0]=total_level_width-32-player.bounding_box[2]
           if player.current_state=="rainbow_walk":
              player_y_axis_collision_rect[1]-=player.yvel
              player.yvel=0
              player.extra_yvel=0                      
        if player_y_axis_collision_rect[1]<32:
           player_y_axis_collision_rect[1]=32
           if player.current_state=="rainbow_walk":
        ...
        for platform in platforms:
            ...
            and platform.rect.bottom>camera_y and platform.rect.top<camera_bottom:
               ...
               if player.yvel > 0 and not player.fall_through:
                  if rect_collision(player_y_axis_collision_rect,platform.rect):
                     if player_y_axis_collision_rect[1]+player.bounding_box[3]-player.yvel<=platform.rect[1]:
                        #player_y_axis_collision_rect.y=platform.rect.top-player_y_axis_collision_rect.height
